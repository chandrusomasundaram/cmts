<?php
	...
	if($count > 0)
	{
	 ...
	 $query = "SELECT * FROM pickup p INNER JOIN customer_registration cr ON p.cus_id = cr.cus_id AND sender_city = '".$city."'";
	 ...
	 if($count >= 1)
	 {
	  while($row = mysqli_fetch_array($result))
	  {
		...
		<td>".$row['cus_name']."</td>
		<td>".$row['book_date']."</td>
		<td class='emailname' data-id=".$row['cus_emailid'].">".$row['cus_emailid']."</td>
		<td>".$row['cus_mob_no']."</td>
		<td>".$row['status']."</td>
